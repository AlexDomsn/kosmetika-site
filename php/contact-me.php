...
if($_POST) {
    ...
    $to_Email = "verticalofis@mail.ru"; // Write your email here
   ...
   /* // Checking if the $_POST vars well provided, Exit if there is one missing
    if(!isset($_POST["userName"]) || !isset($_POST["userEmail"]) || !isset($_POST["userSubject"]) || !isset($_POST["userMessage"])) {
        ...
        $output = json_encode(array('type'=>'error', 'text' => '<i class="icon ion-close-round"></i> Заполните пожалуйста поля!'));
        die($output);
    }*/
   
  // PHP validation for the fields required
    /*if(empty($_POST["userName"])) {
        $output = json_encode(array('type'=>'error', 'text' => '<i class="icon ion-close-round"></i> Напишите пожалуйста ваш номер телефона для связи!'));
        die($output);
    }
    
    if(!filter_var($_POST["userEmail"], FILTER_VALIDATE_EMAIL)) {
        $output = json_encode(array('type'=>'error', 'text' => '<i class="icon ion-close-round"></i>'));
        die($output);
    }
    ...
    $emailcontent = 'Номер телефона: <strong>'.$_POST["userName"].'</strong><br/><br/>'. "\r\n" .
                'His message: <br/> <em>'.$_POST["userMessage"].'</em><br/><br/>'. "\r\n" .
                '<strong>Дополнительная информация: '.$_POST["userEmail"].'</strong>' . "\r\n" ;
    ...
    if(!$Mailsending) {
        ...
        $output = json_encode(array('type'=>'message', 'text' => '<i class="icon ion-checkmark-round"></i> Отправлено! Мы с вами свяжемся в ближайшее время!'));
        die($output);
    }
}
?>